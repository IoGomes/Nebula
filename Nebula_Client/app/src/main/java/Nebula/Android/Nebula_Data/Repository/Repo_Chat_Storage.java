...
public class Repo_Chat_Storage {
    ...
    public void saveMessages(String chatSessionId, List<Entity_Message> messages) {
        JSONArray jsonArray = new JSONArray();

        try {
            for (Entity_Message message : messages) {
    ...
    public List<Entity_Message> loadMessages(String chatSessionId) {
        List<Entity_Message> messages = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                Entity_Message message = new Entity_Message();
    ...
    public void addMessage(String chatSessionId, Entity_Message message) {
        List<Entity_Message> messages = loadMessages(chatSessionId);
        messages.add(message);
